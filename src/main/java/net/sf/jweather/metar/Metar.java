 ...
 */
public class Metar {
	private static org.slf4j.Logger logger = LoggerFactory.getLogger(Metar.class);
	...
	ArrayList<RunwayVisualRange> runwayVisualRanges = new ArrayList<RunwayVisualRange>();
	ArrayList<Obscuration> obscurations = new ArrayList<Obscuration>();
	private boolean isNoSignificantChange = false;
	private String becoming = null;

	public Metar() {
		logger.debug("Metar: instantiated");
	}
	...
	protected void setWindSpeedInMPS(Float value) {
		this.windSpeed = Float.valueOf((float)(value.floatValue() / 0.5148));
	}
	...
	public Float getWindSpeedInMPS() {
		return Float.valueOf((float)(this.windSpeed.floatValue() * 0.5148));
	}
	...
	protected void setWindGustsInMPS(Float value) {
		this.windGusts = Float.valueOf((float)(value.floatValue() / 0.5148));
	}
	...
	public Float getWindGustsInMPS() {
		return Float.valueOf((float)(this.windGusts.floatValue() * 0.5148));
	}
	...
	protected void setIsCavok(boolean value) {
		...
		setVisibilityInKilometers(Float.valueOf(10));
	}
	...
	public Float getVisibility() {
		if (visibilityMiles != null) {
			...
			return Float.valueOf((float)(visibilityKilometers.floatValue() / 1.609344));
		} else if (visibilityMeters != null) {
			return Float.valueOf((float)(visibilityMeters.floatValue() / 1609.344));
		}
		return null;
	}
	...
	public Float getVisibilityInKilometers() {
		if (visibilityKilometers != null) {
			...
			return Float.valueOf((float)(visibilityMeters.floatValue() / 1000));
		} else if (visibilityMiles != null) {
			return Float.valueOf((float)(visibilityMiles.floatValue() * 1.609344));
		}
		return null;
	}
	...
	public Float getVisibilityInMeters() {
		if (visibilityMeters != null) {
			...
			return Float.valueOf((float)(visibilityKilometers.floatValue() * 1000));
		} else if (visibilityKilometers != null) {
			return Float.valueOf((float)(visibilityKilometers.floatValue() * 1609.344));
		}
		return null;
	}
	...
	protected void setPressure(Float value) {
		...
		if( value != null )
		{
			...
			this.pressure = Float.valueOf((float)(val));
		}
		else {
			this.pressure = value;
		}
	...
	public Integer getPressureInHectoPascals() {
		...
		// round
		return Integer.valueOf((Math.round(val)));
	}
	 ...
	 * @see RunwayVisualRange
	 */
	public ArrayList<RunwayVisualRange> getRunwayVisualRanges() {
	 ...
	 * @see Obscuration
	 */
	public ArrayList<Obscuration> getObscurations() {
		return obscurations;
	}
...
//		System.out.println("hour : " + date.getMinutes());
		...
		if (getWeatherConditions() != null) {
			Iterator<WeatherCondition> i = getWeatherConditions().iterator();
			while (i.hasNext()) {
		...
		if (getSkyConditions() != null) {
			Iterator<SkyCondition> i = getSkyConditions().iterator();
			while (i.hasNext()) {
				SkyCondition skyCondition = (SkyCondition)i.next();
				System.out.println(skyCondition.getNaturalLanguageString());
			}
		}
		if (getObscurations() != null) {
			Iterator<Obscuration> i = getObscurations().iterator();
			while (i.hasNext()) {
				Obscuration obs = (Obscuration)i.next();
				System.out.println(obs.getNaturalLanguageString());
			}
		}
		if (getRunwayVisualRanges() != null) {
			Iterator<RunwayVisualRange> i = getRunwayVisualRanges().iterator();
			while (i.hasNext()) {
				RunwayVisualRange runway = (RunwayVisualRange)i.next();
				System.out.println(runway.getNaturalLanguageString());
			}
		}
	}
